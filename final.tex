...
В работе рассматривается граница периодичности, определённая в \cite{maxPlusAtWork}. Приводится алгоритм для вычисления этой характеристики для букета из циклов, то есть для графа, состоящего из ориентированных циклов, пересекающихся по одной вершине. Также получена формула для вычисления границы периодичности для графа-ромашки, то есть для ориентированного цикла, к каждой вершине которого подвешено по циклу.

In this paper we discuss a transient defined by Sergeev in \cite{maxPlusAtWork}. An algorithm is given to calculate transient for a bouquet of cycles (for a graph consisting of oriented cycles intersecting at one vertex). A formula is also obtained for calculating the transient for a chamomile graph (for an oriented cycle, to each vertex of which a cycle is suspended).
...
\begin{tikzpicture}[shorten >=1pt, auto, node distance=3cm, ultra thick]
    ...
    \tikzstyle{edge_style} = [draw=black]
    \node[node_style] (v1) at (0, 0) {1};
    \node[node_style] (v2) at (2.5, 1.5) {2};
    \node[node_style] (v3) at (2.5, -1.5) {3};
    \node[node_style] (v4) at (-1.5, 1.5) {4};
...
\begin{tikzpicture}[shorten >=1pt, auto, node distance=3cm, ultra thick]
    \tikzstyle{node_style} = [circle,draw=black ,font=\sffamily\Large\bfseries]
    \tikzstyle{edge_style} = [draw=black]
    \node[node_style] (v1) at (1, 1) {1};
    \node[node_style] (v2) at (1, -1) {2};
    \node[node_style] (v3) at (-1, 0) {3};
    \node[node_style] (v4) at (2, 2.6) {4};
    \node[node_style] (v5) at (3, 1) {5};
    \node[node_style] (v6) at (3, -1) {6};
    \node[node_style] (v7) at (4, -3) {7};
    \node[node_style] (v8) at (3, -5) {8};
    \node[node_style] (v9) at (1, -5) {9};
    \node[node_style] (v10) at (0, -3) {10};
    \node[node_style] (v11) at (-2, 2) {11};
    \node[node_style] (v12) at (-4, 2) {12};
    \node[node_style] (v13) at (-5, 0) {13};
    \node[node_style] (v14) at (-4, -2) {14};
    \node[node_style] (v15) at (-2, -2) {15};
	...
	\path[->, bend right] (v3) edge (v11)
    			      	    ...
    			      	    (v15) edge (v3);
\end{tikzpicture}
\caption{Граф-ромашка с $n = 3, k_1 = 1, k_2 = 2, k_3 = 2$.}
\end{figure}
