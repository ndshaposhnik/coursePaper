...
Рассмотрим матрицу смежности $A$ графа-ромашки $\mathcal{G}(A)$ на $p(k - 1) + 1$ вершине, состоящего из $p > 1$ циклов длины $k$. Будем считать, что вершина, по которой пересекаются все циклы, имеет номер $1$, и что все рёбра в этом графе имеют нулевой вес.
...
\section{Конусы}
\begin{definition}
Пусть $K$ --- некоторе непустое подмножество $\mathbb{R}^n$. Назовём $K$ конусом, если выполнены следующие условия:
\begin{enumerate}
...
\begin{definition}
Подконус $F$ конуса $K$ называется гранью конуса $K$, если для любых $x \in F$, $y \in K$ из $x \ge^K y \ge^K 0$ следует $y \in F$. Обозначим множество всех граней конуса $K$ через $\mathcal{F}(K)$.

Для любого подмножества $S \subset K$ определим $\Phi(S)$ --- наименьшая по включению грань конуса $K$, содержащее $S$:\begin{equation}
\Phi(S) = \cap \{ F \ | \ F \text{--- грань $K$ и $F \supseteq S$} \}
\end{equation}
Если $S = \{ x \}$, то для удобства будем писать $\Phi(x) = \Phi(\{ x \})$
\end{definition}

Заметим, что $\{ 0 \}$ и $K$ являются гранями конуса $K$. Назовём эти грани тривиальными, а все остальные --- нетривиальными. Обозначим множество всех нетривиальных граней конуса $K$ через $\mathcal{F}'(K)$.

Иногда для описания некоторой грани $F$ конуса $K$ достаточно взять один вектор из $F$. Обозначим через $ri F$ относительную внутренность   множества $F$. (см. \cite{LoewyTam}).
...
Таким образом, экстремальные лучи --- это одномерные грани конуса. Простейший пример полиэдрального конуса --- это положительный октант $\mathbb{R}^n_+ = \{ (\xi_1, \dots, \xi_n)^R \ | \ \xi_i \ge 0 \text{ для любого $1 \le i \le n$} \}$. Этот конус является симплициальным. Заметим, что любой симплициальный конус линейно изоморфен $\mathbb{R}^n_+$ (см. \cite{LoewyTam}).
...
$A$ называется $K$-неразложимой, если любая нетривиальная грань конуса $K$ не инвариантно относительно $K$.
...
По конусу $K$ и матрицы $A$, его сохраняющей, можно определить несколько графов. Рассмотрим две грани $F, G$ конуса $K$. Будем говорить, что от $F$ до $G$ идёт $\mathcal{P}$-ребро, если $G \subseteq \Phi(AF)$, и $\mathcal{I}$-ребро, если $G \subseteq \Phi((I + A)F)$, где $I$ --- единичная матрица подходящей размерности. Заметим, что если от $F$ до $G$ идёт $\mathcal{P}$-ребро, то идёт и $\mathcal{I}$-ребро (см. \cite{LoewyTam}).
...
Пусть $F, G$ --- грани конуса $K$. Тогда в графе $(\mathcal{F}, \mathcal{P}(A, K))$ (или $(\mathcal{F}, \mathcal{I}(A, K))$) существует путь длины $k$ между $F$ и $G$ тогда и только тогда, когда $G \subseteq \Phi(A^kF)$ (соответственно, $G \subseteq \Phi((I + A)^kF)$.
\end{proposition}
...
\begin{lemma}
\label{lemmaFK}
Если некоторая грань $F$ полиэдрального конуса $K$ содержит все нетривиальные грани $K$, то $F = K$.
\end{lemma}
\begin{proof}[\textbf{Доказательство}]
...
\end{theorem}
\begin{proof}[\textbf{Доказательство}]
Пусть $q = \gamma(A)$. Тогда заметим, что для любой нетривиальной грани $F$ конуса $K$ верно равенство $\Phi(A^qF) = K$. Это следует из следующего: рассмотрим произвольный ненулевой вектор $x \in F$. Тогда, так как $A$ примитивна, $A^q x \gg 0$. Значит, $\Phi(A^q F) \supseteq \Phi(A^q x) = K$ и, следовательно, $\Phi(A^q F) = K$.

Значит, для любых нетривиальных граней $F, G$ конуса $K$ в $(\mathcal{F}', \mathcal{P})$ существует путь между $F$ и $G$ длины $p$ --- это равносильно $G \subseteq \Phi(A^q F) = K$, а любая грань конуса в нём содержится. Значит, экспонента графа $(\mathcal{F}', \mathcal{P})$ не превосходит экспоненты матрицы $A$, то есть $exp(\mathcal{F}', \mathcal{P}) \le \gamma(A)$.
...
Покажем, что $A^p x \in int K$ для экстремальных векторов. Тогда в $(\mathcal{F}', \mathcal{P})$ существует путь из $\Phi(x)$ в любую другую вершину. Следовательно, $\Phi(A^p \Phi(x))$ содержит в себе все нетривиальные грани конуса, а значит $\Phi(A^p \Phi(x)) = K$. (по лемме \ref{lemmaFK}). Докажем следующую лемму: 
...
Значит, $A^p x_1 \in \Phi(A^p x)$ и $\Phi(A^p x_1) \subseteq \Phi(A^p x)$. Но по лемме \ref{facesOfPower} $\Phi(A^p x_1) = K$. Следовательно, $\Phi(A^p x) = K$. Таким образом, $A^p x \in int K$.
...
\end{comment}
    ...
    \item $\Phi(S)$ --- грань конуса, порождённая множеством $S$.
    
    $\Phi(x) = \Phi(\{ x \}$ --- грань конуса, порождённая одноэлементным множеством.
